...
for f in $data_root/${train_set}/feats.scp ; do
  if [ ! -f $f ]; then
...
if [ $stage -le 2 ] && [ -f $data_root/${train_set}_sp_hires/feats.scp ]; then
  echo "$0: $data_root/${train_set}_sp_hires/feats.scp already exists."
...
if [ $stage -le 1 ]; then
  ...
  for datadir in ${train_set} ${ivector_train_set}; do
    utils/data/perturb_data_dir_speed_3way.sh $data_root/${datadir} $data_root/${datadir}_sp
  done
fi
...
if [ $stage -le 2 ]; then
  ...
  mfccdir=$data_root/${train_set}_sp_hires/data
  ...
  for datadir in ${train_set} ${ivector_train_set}; do
    utils/copy_data_dir.sh $data_root/${datadir}_sp $data_root/${datadir}_sp_hires
	# do volume-perturbation on the training data prior to extracting hires
    # features; this helps make trained nnets more invariant to test data volume.
    utils/data/perturb_data_dir_volume.sh $data_root/${datadir}_sp_hires
	# make mfcc features
    steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
                                --cmd "$train_cmd" $data_root/${datadir}_sp_hires
    steps/compute_cmvn_stats.sh $data_root/${datadir}_sp_hires
    utils/fix_data_dir.sh $data_root/${datadir}_sp_hires
  done
  
  for datadir in dev eval ; do
    utils/copy_data_dir.sh $data_root/../$datadir $data_root/${datadir}_hires
	# make mfcc features
    steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
                                  --cmd "$train_cmd" $data_root/${datadir}_hires
    steps/compute_cmvn_stats.sh $data_root/${datadir}_hires
    utils/fix_data_dir.sh $data_root/${datadir}_hires
  done
fi
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set} ${ivector_train_set}; do
	utils/data/combine_short_segments.sh \
	 $data_root/${datadir}_sp_hires $min_seg_len $data_root/${datadir}_sp_hires_comb

	# just copy over the CMVN to avoid having to recompute it.
	cp $data_root/${datadir}_sp_hires/cmvn.scp $data_root/${datadir}_sp_hires_comb/
	  utils/fix_data_dir.sh $data_root/${datadir}_sp_hires_comb
  done
fi

# modify only ivector training set
...
if [ $stage -le 4 ]; then
  ...
  utils/data/subset_data_dir.sh --utt-list $data_root/${ivector_train_set}/feats.scp \
          $data_root/${ivector_train_set}_sp_hires $temp_data_root/${ivector_train_set}_hires

  # note: essentially all the original segments should be in the hires data.
  n1=$(wc -l <$data_root/${ivector_train_set}/feats.scp)
  n2=$(wc -l <$temp_data_root/${ivector_train_set}_hires/feats.scp)
  if [ $n1 != $n1 ]; then
  ...
  case $ivector_transform_type in
    lda)
      ...
      steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 7 --mllt-iters "2 4 6" \
        ...
        3000 10000 $temp_data_root/${ivector_train_set}_hires data/lang \
    ...
    pca)
      ...
      steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
        ...
        $temp_data_root/${ivector_train_set}_hires \
...
if [ $stage -le 5 ]; then
  ...
  num_utts_total=$(wc -l <$data_root/${ivector_train_set}_sp_hires/utt2spk)
  num_utts=$[$num_utts_total/4]
  utils/data/subset_data_dir.sh $data_root/${ivector_train_set}_sp_hires \
      $num_utts ${temp_data_root}/${ivector_train_set}_sp_hires_subset
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    ${temp_data_root}/${ivector_train_set}_sp_hires_subset 512 \
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 --num_threads 8 --num_processes 1 \
    $data_root/${ivector_train_set}_sp_hires $exp_root/nnet3${nnet3_affix}/diag_ubm $exp_root/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 7 ]; then
  ...
  ivectordir=$exp_root/nnet3${nnet3_affix}/ivectors_${ivector_train_set}_sp_hires_comb
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    $data_root/${ivector_train_set}_sp_hires_comb ${temp_data_root}/${ivector_train_set}_sp_hires_comb_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd --num_threads 8" --nj $nj \
    ${temp_data_root}/${ivector_train_set}_sp_hires_comb_max2 \
  ...
  for datadir in dev eval; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd --num_threads 8" --nj "$nj" \
        $data_root/${datadir}_hires $exp_root/nnet3${nnet3_affix}/extractor \
...
if [ $stage -le 8 ]; then
  if [ "$train_set" != "$ivector_train_set" ]; then
      ...
      steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd --num_threads 8" --nj $nj \
        ${temp_data_root}/${train_set}_sp_hires_comb_max2 $exp_root/nnet3${nnet3_affix}/extractor \
