...
if [ $stage -le 10 ]; then
  local/semisup/chain/run_tdnn_20k_semisupervised.sh \
    ...
    --sup-lat-dir $exp_root/chain_semi20k_80k${clean_affix}/${gmm}_${train_sup_dir}${clean_affix}_sp_comb_unk_lats \
    --sup-tree-dir $exp_root/chain_semi20k_80k${clean_affix}/tree_bi_unk_a \
    --ivector-root-dir $exp_root/nnet3_semi20k_80k${clean_affix} \
    --chain-affix _semi20k_80k${clean_affix} \
	--data_root $data_root \
    --exp-root $exp_root --stage 14
fi
...
if [ $stage -le 11 ]; then
  local/semisup/chain/run_tdnn.sh \
    ...
    --chain-affix _semi20k_80k${clean_affix} --tdnn-affix 1b_oracle${clean_affix} --nj 100 \
    --gmm $gmm --exp_root $exp_root --data_root $data_root --stage 14
    #--stage 8 || exit 1
fi
