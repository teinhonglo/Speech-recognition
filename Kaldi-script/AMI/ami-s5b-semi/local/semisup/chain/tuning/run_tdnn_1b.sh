...
if $use_ihm_ali; then
  ...
  ali_dir=$exp_root/${gmm}_ali_${train_set}_sp_comb_unk
  ...
  lat_dir=$exp_root/chain${chain_affix}/${gmm}_${train_set}_sp_comb_unk_lats
...
if [ $stage -le 13 ]; then
  ...
  if [ $lang/L.fst -nt data/lang_unk/L.fst ]; then
