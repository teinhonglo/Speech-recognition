...
data=$data_root/${train_set}
cleaned_data=$data_root/${train_set}_${cleanup_affix}
srcdir=$exp_root/$gmm
dir=$exp_root/${gmm}_${cleanup_affix}_work 
...
if [ $stage -le 2 ] && [ $mic != "ihm" ]; then
  ...
  utils/data/subsegment_data_dir.sh --segment-end-padding $padding \
    $data_root/train_ihmdata $dir/segments $dir/text $data_root/${train_set}_${cleanup_affix}_ihmdata
  # note, there will be no feats or CMVN in these directories.
fi

if [ $stage -le 3 ]; then
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd --num_threads 3" \
    $data_root/${train_set}_${cleanup_affix} data/lang $exp_root/$gmm $exp_root/${gmm}_ali_${cleanup_affix}
fi

if [ $stage -le 4 ]; then
  steps/train_sat.sh --cmd "$train_cmd --num_threads 5" --stage "$train_stage" \
    5000 80000 $data_root/${train_set}_${cleanup_affix} data/lang $exp_root/${gmm}_ali_${cleanup_affix} $exp_root/${gmm}_${cleanup_affix}
fi
...
if [ $stage -le 5 ]; then
  graph_dir=$exp_root/${gmm}_${cleanup_affix}/graph_$LM
