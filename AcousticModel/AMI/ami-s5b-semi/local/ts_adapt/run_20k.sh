...
if [ $stage -le 10 ]; then
  ...
  local/ts_adapt/chain/tuning/run_tdnn_20k_semisupervised_semits_1a.sh \
    --mic $mic \
    --supervised-set ${train_sup_dir}${clean_affix} \
    --unsupervised-set semisup20k_80k \
    --sup-chain-dir $exp_root/chain_semi20k_80k${clean_affix}/tdnn_${tuning_affix}_sp_bi_semisup_1a \
