...
tdnn_affix=_whole_semisup_ts_1a
...
# Input seed system
sup_chain_dirs=($exp_root/chain_semi20k_80k/tdnn_1b_sp_bi_semisup_1a \
                $exp_root/chain_semi20k_80k/tdnn_1a_sp_bi_semisup_1b \
		$exp_root/chain_semi20k_80k/tdnn_1c_sp_bi_semisup_1c \
		$exp_root/chain_semi20k_80k/tdnn_1d_sp_bi_semisup_1d)
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet3/decode_score_fusion.sh --num-threads 5 --nj $nj --cmd "$decode_cmd" \
	    ...
	    --frame-subsampling-factor 3 --use-gpu true \
            $graphdir $data_root/${unsupervised_set_perturbed}_hires_comb ${sup_chain_dirs[@]} $sup_chain_dir/decode_semi_fusion_${unsupervised_set_perturbed}
fi
...
if [ $stage -le 5 ]; then
  steps/lmrescore_const_arpa_undeterminized.sh --cmd "$decode_cmd" \
    ...
    $sup_chain_dir/decode_semi_fusion_${unsupervised_set_perturbed} \
    $sup_chain_dir/decode_semi_fusion_${unsupervised_set_perturbed}_big
  ln -sf ../final.mdl $sup_chain_dir/decode_semi_fusion_${unsupervised_set_perturbed}_big/final.mdl
fi
...
if [ $stage -le 8 ]; then
  steps/best_path_weights.sh --cmd "${train_cmd}" --acwt 0.1 \
    $data_root/${unsupervised_set_perturbed}_hires_comb \
    $sup_chain_dir/decode_semi_fusion_${unsupervised_set_perturbed}_big \
    $sup_chain_dir/best_path_semi_fusion_${unsupervised_set_perturbed}_big
fi
...
if [ $stage -le 10 ]; then
  steps/nnet3/chain/make_weighted_den_fst.sh --num-repeats $lm_weights --cmd "$train_cmd" \
    ${sup_tree_dir} ${sup_chain_dir}/best_path_semi_fusion_${unsupervised_set_perturbed}_big \
    $dir
fi
...
unsup_lat_dir=${sup_chain_dir}/decode_semi_fusion_${unsupervised_set_perturbed}_big
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh \
      ...
      --deriv-weights-scp $sup_chain_dir/best_path_semi_fusion_${unsupervised_set_perturbed}_big/weights.scp \
      --online-ivector-dir $ivector_root_dir/ivectors_${unsupervised_set_perturbed}_hires_comb \
...
comb_egs_dir=$dir/comb_egs${phn_affix}${supervision_affix}
if [ $stage -le 14 ]; then
...
test_graph_affix=${test_graph_affix}${phn_affix}${supervision_affix}
