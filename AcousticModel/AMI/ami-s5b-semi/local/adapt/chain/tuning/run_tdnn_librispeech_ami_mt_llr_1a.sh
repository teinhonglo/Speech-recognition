...
# !!!!
ami_lat_dir=$exp_root/chain${chain_affix}/${gmm}_${ami_lores_dir}_comb_lats
ami_ivector_dir=$ivector_root_dir/ivectors_${ami_hires_dir}_comb

ls_lores_dir=train_ls_sp
ls_hires_dir=${ls_lores_dir}_hires
ls_lat_dir=$exp_root/chain${chain_affix}/${gmm}_${ls_lores_dir}_comb_lats
ls_ivector_dir=$ivector_root_dir/ivectors_${ls_hires_dir}_comb

gmm_dir=$exp_root/${gmm}
ali_dir=$exp_root/${gmm}_ali_train_sup_sp_comb

tree_dir=$exp_root/chain_smei20k_80k/tree_bi${tree_affix}
dir=$exp_root/chain${chain_affix}/tdnn_librispeech_ami_mt_llr_1a
...
if [ $stage -le 1 ]; then
  for data_dir in $ami_lores_dir $ami_hires_dir $ls_lores_dir $ls_hires_dir; do
...
if [ $stage -le 2 ]; then
  # extractor i-vector
  for data_dir in  $ami_hires_dir $ls_hires_dir; do
...
if [ $stage -le 3 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" --generate_ali_from_lats true $data_root/${ami_lores_dir}_comb \
    data/lang $gmm_dir $ami_lat_dir || exit 1;
  rm $ami_lat_dir/fsts.*.gz 2>/dev/null || true # save space

  # gernarting lattice for librispeech.
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" --generate_ali_from_lats true $data_root/${ls_lores_dir}_comb \
    data/lang $gmm_dir $ls_lat_dir || exit 1;
  rm $ls_lat_dir/fsts.*.gz 2>/dev/null || true # save space
fi
...
if [ -z "$common_treedir" ]; then
    ...
    steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
       ...
       --cmd "$train_cmd" 4200 $data_root/$ami_lores_dir $lang $ami_lat_dir $tree_dir
   fi
else
...
echo "egs_left_context $egs_left_context, egs_right_context  $egs_right_context"

egs_dir=$dir/egs

if [ -z "$ami_egs_dir" ]; then
  # ami
  if [ $stage -le 8 ]; then
    ami_egs_dir=${egs_dir}_ami
    mkdir -p $ami_egs_dir
    touch $ami_egs_dir/.nodelete # keep egs around when that run dies.

    echo "$0: generating egs from the ami data"
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
               ...
               --cmvn-opts "$cmvn_opts" \
               --online-ivector-dir $ami_ivector_dir \
               --generate-egs-scp true \
               $data_root/${ami_hires_dir}_comb $dir \
               $ami_lat_dir $ami_egs_dir
  fi
fi

if [ -z "$ls_egs_dir" ]; then
  if [ $stage -le 9 ]; then
    # librispeech
    ls_egs_dir=${egs_dir}_ls
    mkdir -p ${ls_egs_dir}
    touch $ls_egs_dir/.nodelete # keep egs around when that run dies.

    echo "$0: generating egs from the librispeech data"
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
               --left-context $egs_left_context --right-context $egs_right_context \
               --frame-subsampling-factor $frame_subsampling_factor \
               --alignment-subsampling-factor $frame_subsampling_factor \
               ...
               --frames-per-iter 1500000 \
               --cmvn-opts "$cmvn_opts" \
               --online-ivector-dir $ls_ivector_dir \
               --generate-egs-scp true \
               $data_root/${ls_hires_dir}_comb $dir \
               $ls_lat_dir $ls_egs_dir
  fi
fi
...
if [ $stage -le 10 ]; then
  steps/nnet3/chain/multilingual/combine_egs.sh --cmd "$train_cmd" \
    --block-size 128 \
    --lang2weight $transfer_weights 2 \
    $ami_egs_dir $ls_egs_dir $comb_egs_dir
...
if [ $stage -le 11 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    --cmd "$cuda_cmd" \
    --feat.online-ivector-dir "$ami_ivector_dir" \
    ...
    --feat-dir $data_root/$ami_hires_dir \
    --tree-dir $tree_dir \
    --lat-dir $ami_lat_dir \
    --dir $dir || exit 1;
fi
