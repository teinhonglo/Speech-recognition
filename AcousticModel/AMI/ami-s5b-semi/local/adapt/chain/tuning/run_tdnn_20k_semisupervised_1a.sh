...
# Input seed system
sup_chain_dir=$exp_root/chain_semi20k_100k_250k/tdnn_1a_sp  # supervised chain system
sup_lat_dir=$exp_root/chain_semi20k_100k_250k/tri4a_train_sup50k_unk_lats  # lattices for supervised set
sup_tree_dir=$exp_root/chain_semi20k_100k_250k/tree_bi_a  # tree directory for supervised chain system
ivector_root_dir=$exp_root/nnet3_semi20k_100k_250k  # i-vector extractor root directory
...
for f in $data_root/${supervised_set_perturbed}/feats.scp \
  $data_root/${supervised_set_perturbed}_hires/feats.scp \
...
if [ $stage -le 3 ]; then
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    $data_root/${unsupervised_set_perturbed}_hires $data_root/${unsupervised_set_perturbed}_max2_hires

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    $data_root/${unsupervised_set_perturbed}_max2_hires $ivector_root_dir/extractor \
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet3/decode_semisup.sh --num-threads 4 --nj $nj --cmd "$decode_cmd" \
            ...
            $graphdir $data_root/${unsupervised_set_perturbed}_hires $sup_chain_dir/decode_${unsupervised_set_perturbed}
fi
...
if [ -z "$sup_egs_dir" ]; then
  ...
  if [ $stage -le 12 ]; then
    ...
    steps/nnet3/chain/get_egs.sh --cmd "$decode_cmd" \
               ...
               $data_root/${supervised_set_perturbed}_hires $dir \
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh \
      ...
      $data_root/${unsupervised_set_perturbed}_hires $dir \
