...
# to generate alignments for AMI using Librispeech model.
...
#    in Librispeech lexicon.txt. ami words, that are not presented in Librispeech, are added as oov
#    in new lexicon.txt.
# 2) It uses Librispeech tree-dir and generates new alignments and lattices for ami using
#    Librispeech gmm model.
# 3) It also trains phone LM using weighted combination of alignemts from Librispeech
#    and ami, which is used in chain denominator graph.
...
# and use already trained model on Librispeech and fine-tune the whole network using
# ami data while training the last layer with higher learning-rate.
# The chain config is as run_tdnn_5n.sh and the result is:
# System tdnn_5n tdnn_Librispeech_ami_1a tdnn_Librispeech_ami_1b tdnn_Librispeech_ami_1c
...
phone_lm_scales="1,10" # comma-separated list of positive integer multiplicities
                       ...
                       # to give the AMI data a higher weight).
...
src_mdl=../../Librispeech/s5/exp/chain_cleaned/tdnn_1b_sp/final.mdl # input chain model
                                                    # trained on source dataset (Librispeech) and
...
lang_src_tgt=data/lang_Librispeech_ami # This dir is prepared using phones.txt and lexicon from
                              # source(Librispeech) and and wordlist and G.fst from target(AMI)
lat_dir=$exp_root/chain${chain_affix}/${gmm}_${train_set}_sp_comb_lats_librispeech_dnn
dir=$exp_root/chain${chain_affix}/tdnn_librispeech_ami${tdnn_affix}_dnn
...
if [ $stage -le -1 ]; then
  echo "$0: Prepare lang for AMI-Librispeech using Librispeech phone set and lexicon and AMI word list."
  if ! cmp -s <(grep -v "^#" $src_lang/phones.txt) <(grep -v "^#" $lang_dir/phones.txt); then
    local/prepare_Librispeech_ami_lang.sh  $src_dict $src_lang $lang_src_tgt || exit 1;
  else
...
ivec_opt=""
if $use_ivector;then ivec_opt="--online-ivector-dir exp/nnet3${nnet3_affix}/ivectors" ; fi
...
if [ $stage -le 6 ]; then
  echo "$0: compute {den,noamialization}.fst using weighted phone LM."
...
if [ $stage -le 7 ]; then
  ...
  ivector_dir=
  if $use_ivector; then ivector_dir="exp/nnet2${nnet3_affix}/ivectors" ; fi
  ...
  steps/nnet3/chain/train.py --stage $train_stage ${chain_opts[@]} \
    ...
    --feat.cmvn-opts "--noami-means=false --noami-vars=false" \
    --chain.xent-regularize 0.1 \
...
if [ $stage -le 8 ]; then
  ...
  # the lang directory.
  tes_ivec_opt=""
  if $use_ivector;then test_ivec_opt="--online-ivector-dir exp/nnet2${nnet3_affix}/ivectors_test" ; fi
