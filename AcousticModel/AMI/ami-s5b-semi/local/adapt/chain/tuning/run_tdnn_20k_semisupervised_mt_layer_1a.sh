...
lm_weights=3,2,2,2,2  # Weights on phone counts from supervised, unsupervised data for denominator FST creation
...
if [ $stage -le 10 ]; then
  steps/nnet3/chain/make_weighted_den_fst.sh --num-repeats $lm_weights --cmd "$train_cmd" \
	...
	${sup_chain_dirs[1]}/best_path_${unsupervised_set_perturbed}_big \
	${sup_chain_dirs[2]}/best_path_${unsupervised_set_perturbed}_big \
	${sup_chain_dirs[3]}/best_path_${unsupervised_set_perturbed}_big \
    $dir
fi
