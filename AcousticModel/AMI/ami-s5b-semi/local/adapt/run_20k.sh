...
if [ $stage -le 9 ]; then
  local/adapt/chain/run_tdnn_librispeech_to_ami.sh \
...
if [ $stage -le 10 ]; then
  local/adapt/chain/run_tdnn_20k_semisupervised.sh \
    --mic $mic \
    --supervised-set ${train_sup_dir}${clean_affix} \
    --unsupervised-set ${train_unsup_dir}${clean_affix} \
    --sup-chain-dir $exp_root/chain_online_librispeech${clean_affix}/tdnn_librispeech_ami_${tuning_affix} \
    --sup-lat-dir $exp_root/chain_online_librispeech${clean_affix}/${gmm}_${train_sup_dir}${clean_affix}_sp_comb_lats \
    --sup-tree-dir $exp_root/chain_online_librispeech${clean_affix}/tree_bi \
    --ivector-root-dir $exp_root/nnet3_online_librispeech \
    --chain-affix _online_librispeech${clean_affix} \
    --data-root $data_root \
