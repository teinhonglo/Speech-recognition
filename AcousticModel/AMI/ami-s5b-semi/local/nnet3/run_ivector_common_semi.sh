...
if [ $stage -le 2 ]; then
  ...
  for datadir in ${train_set}_sp ; do
    ...
    utils/data/perturb_data_dir_volume.sh $data_root/${datadir}_hires
	# make mfcc features
...
if [ $stage -le 3 ]; then
  ...
  utils/data/combine_short_segments.sh \
     $data_root/${train_set}_sp_hires $min_seg_len $data_root/${train_set}_sp_hires_comb

  # just copy over the CMVN to avoid having to recompute it.
  cp $data_root/${train_set}_sp_hires/cmvn.scp $data_root/${train_set}_sp_hires_comb/
  utils/fix_data_dir.sh $data_root/${train_set}_sp_hires_comb/
fi
...
if [ $stage -le 4 ]; then
  ...
  utils/data/subset_data_dir.sh --utt-list $data_root/${train_set}/feats.scp \
          $data_root/${train_set}_sp_hires $temp_data_root/${train_set}_hires

  # note: essentially all the original segments should be in the hires data.
  n1=$(wc -l <$data_root/${train_set}/feats.scp)
...
if [ $stage -le 5 ]; then
  ...
  num_utts_total=$(wc -l <$data_root/${train_set}_sp_hires/utt2spk)
  num_utts=$[$num_utts_total/4]
  utils/data/subset_data_dir.sh $data_root/${train_set}_sp_hires \
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 --init_iter 0 --num_threads 8 --num_processes 1 \
    $data_root/${train_set}_sp_hires $exp_root/nnet3${nnet3_affix}/diag_ubm $exp_root/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 7 ]; then
  ...
  for data in dev eval; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "$nj" \
      $data_root/${data}_hires $exp_root/nnet3${nnet3_affix}/extractor \
