...
train_unsup_dir=train_unsup90k
semi_dir=semisup_10k
exp_root=exp/$mic/semisup_10k
...
if [ $stage -le 1 ]; then
  steps/train_mono.sh --nj $nj --cmd "$train_cmd" \
    $data_root/${train_sup_dir} data/lang $exp_root/mono || exit 1
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
    $data_root/${train_sup_dir} data/lang $exp_root/mono $exp_root/mono_ali || exit 1	
fi

if [ $stage -le 2 ]; then
  # tri1
  steps/train_deltas.sh --cmd "$train_cmd" \
    5000 80000 $data_root/${train_sup_dir} data/lang $exp_root/mono_ali $exp_root/tri1 || exit 1
  
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
   $data_root/${train_sup_dir} data/lang $exp_root/tri1 $exp_root/tri1_ali || exit 1;
...
if [ $stage -le 4 ]; then
  ...
  steps/decode_fmllr.sh --stage 3 --nj $nj --cmd "$decode_cmd" --config conf/decode.conf \
...
if [ $stage -le 5 ]; then
  utils/combine_data.sh $data_root/semisup10k_90k \
    $data_root/$train_sup_dir $data_root/$train_unsup_dir
fi
...
WORD
  mkdir -p data/local/pocolm_ex90k

  utils/filter_scp.pl --exclude $data_root/$train_unsup_dir/utt2spk \
    data/$mic/train/text > data/local/pocolm_ex90k/text.tmp

  if [ ! -f data/lang_test_poco_ex90k_big/G.carpa ]; then
    local/fisher_train_lms_pocolm.sh \
      --text data/local/pocolm_ex90k/text.tmp \
      --dir data/local/pocolm_ex90k \
	  --num-dev-sentences 5000 \
      --data-root $data_root

    local/fisher_create_test_lang.sh \
      --arpa-lm data/local/pocolm_ex90k/data/arpa/4gram_small.arpa.gz \
      --dir data/lang_test_poco_ex90k
    utils/build_const_arpa_lm.sh \
      data/local/pocolm_ex90k/data/arpa/4gram_small.arpa.gz \
      data/lang_test_poco_ex90k data/lang_test_poco_ex90k_big
  fi  
fi
...
echo "Prepare lang directories with UNK modeled using phone LM"
  ...
  for lang_dir in data/lang_test_poco_ex90k; do
...
if [ $stage -le 9 ]; then
  local/semisup/chain/run_tdnn.sh \
    ...
    --nnet3-affix _semi10k_90k${clean_affix} \
    --chain-affix _semi10k_90k${clean_affix} \
    --gmm $gmm --exp-root $exp_root --data-root $data_root \
    --ivector-train-set semisup10k_90k${clean_affix} --stage 13
fi
...
if [ $stage -le 10 ]; then
  local/semisup/chain/run_tdnn_10k_semisupervised.sh \
    --mic $mic \
    --supervised-set ${train_sup_dir}${clean_affix} \
    --unsupervised-set ${train_unsup_dir}${clean_affix} \
    --sup-chain-dir $exp_root/chain_semi10k_90k${clean_affix}/tdnn_${tuning_affix}_sp_bi \
    --sup-lat-dir $exp_root/chain_semi10k_90k${clean_affix}/${gmm}_${train_sup_dir}${clean_affix}_sp_comb_lats \
    --sup-tree-dir $exp_root/chain_semi10k_90k${clean_affix}/tree_bi_a \
    --ivector-root-dir $exp_root/nnet3_semi10k_90k${clean_affix} \
    --chain-affix _semi10k_90k${clean_affix} \
    --data-root $data_root \
    --exp-root $exp_root --stage 15
	exit 0;
fi
...
if [ $stage -le 11 ]; then
  local/semisup/chain/run_tdnn.sh \
    --mic $mic \
    --train-set semisup10k_90k${clean_affix} \
    --nnet3-affix _semi10k_90k${clean_affix} \
    --common-treedir $exp_root/chain_semi10k_90k${clean_affix}/tree_bi_a \
    --chain-affix _semi10k_90k${clean_affix} --tdnn-affix _${tuning_affix}_oracle${clean_affix} --nj 100 \
    --gmm $gmm --exp-root $exp_root --data-root $data_root --stage 13
fi
