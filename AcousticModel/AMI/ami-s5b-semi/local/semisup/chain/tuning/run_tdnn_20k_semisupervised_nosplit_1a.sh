...
tdnn_affix=_semisup_nosplit_1a
...
if [ $stage -le 2 ]; then
  utils/data/extract_wav_segments_data_dir.sh $data_root/${unsupervised_set} $data_root/${unsupervised_set}_noseg
  rm $data_root/${unsupervised_set}_noseg/utt2dur
  utils/data/get_utt2dur.sh $data_root/${unsupervised_set}_noseg  # necessary for the next command
  #utils/data/extract_wav_segments_data_dir.sh $data_root/${unsupervised_set} $data_root/${unsupervised_set}_noseg
  
  # 20 in the following command means the allowed lengths are spaced
  # by 20% change in length.
  utils/data/perturb_speed_to_allowed_lengths.py 20 $data_root/${unsupervised_set}_noseg \
                                                 $data_root/${unsupervised_set_perturbed}e2e_hires
  cat $data_root/${unsupervised_set_perturbed}e2e_hires/utt2dur | \
    awk '{print $1 " " substr($1,5)}' >$data_root/${unsupervised_set_perturbed}e2e_hires/utt2uniq
  utils/fix_data_dir.sh $data_root/${unsupervised_set_perturbed}e2e_hires
  
  steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj \
    --mfcc-config conf/mfcc_hires.conf \
    $data_root/${unsupervised_set_perturbed}e2e_hires
  steps/compute_cmvn_stats.sh $data_root/${unsupervised_set_perturbed}e2e_hires
fi
...
if [ $stage -le 3 ]; then
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    $data_root/${unsupervised_set_perturbed}e2e_hires $temp_data_root/${unsupervised_set_perturbed}e2e_max2_hires

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    $temp_data_root/${unsupervised_set_perturbed}e2e_max2_hires $ivector_root_dir/extractor \
    $ivector_root_dir/ivectors_${unsupervised_set_perturbed}e2e_hires || exit 1
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet3/decode_semisup.sh --num-threads 5 --nj $nj --cmd "$decode_cmd" \
			...
			--online-ivector-dir $ivector_root_dir/ivectors_${unsupervised_set_perturbed}e2e_hires \
            --scoring-opts "--min-lmwt 10 --max-lmwt 10" --word-determinize false \
            $graphdir $data_root/${unsupervised_set_perturbed}e2e_hires $sup_chain_dir/decode_${unsupervised_set_perturbed}e2e
fi
...
if [ $stage -le 5 ]; then
  steps/lmrescore_const_arpa_undeterminized.sh --cmd "$decode_cmd" \
    ...
    $data_root/${unsupervised_set_perturbed}e2e_hires \
    $sup_chain_dir/decode_${unsupervised_set_perturbed}e2e \
    $sup_chain_dir/decode_${unsupervised_set_perturbed}e2e_big
  ln -sf ../final.mdl $sup_chain_dir/decode_${unsupervised_set_perturbed}e2e_big/final.mdl
fi
...
if [ $stage -le 8 ]; then
  steps/best_path_weights.sh --cmd "${train_cmd}" --acwt 0.1 \
    $data_root/${unsupervised_set_perturbed}e2e_hires \
    $sup_chain_dir/decode_${unsupervised_set_perturbed}e2e_big \
    $sup_chain_dir/best_path_${unsupervised_set_perturbed}e2e_big
fi
...
if [ $stage -le 10 ]; then
  steps/nnet3/chain/make_weighted_den_fst.sh --num-repeats $lm_weights --cmd "$train_cmd" \
    ${sup_tree_dir} ${sup_chain_dir}/best_path_${unsupervised_set_perturbed}e2e_big \
    $dir
fi
...
unsup_lat_dir=${sup_chain_dir}/decode_${unsupervised_set_perturbed}e2e_big
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh \
      ...
      --deriv-weights-scp $sup_chain_dir/best_path_${unsupervised_set_perturbed}e2e_big/weights.scp \
      --online-ivector-dir $ivector_root_dir/ivectors_${unsupervised_set_perturbed}e2e_hires \
      --generate-egs-scp true $unsup_egs_opts \
      $data_root/${unsupervised_set_perturbed}e2e_hires $dir \
