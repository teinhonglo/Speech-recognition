...
for datadir in $train_set $ivector_train_set ; do
  local/nnet3/run_ivector_common_semi.sh --stage $stage \
                                         ...
                                         --mic $mic --nj $nj --min-seg-len $min_seg_len \
                                         --train-set $datadir \
                                         --gmm $gmm \
...
if [ "$train_set" != "$ivector_train_set" ]; then
  if [ $stage -le 9 ]; then
    ...
    utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
      $data_root/${train_set}_sp_hires $data_root/${train_set}_sp_max2_hires

    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
      $data_root/${train_set}_sp_max2_hires $exp_root/nnet3${nnet3_affix}/extractor \
...
if [ $stage -le 10 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" \
    --generate-ali-from-lats true $data_root/${train_set}_sp \
...
if [ -z "$common_treedir" ]; then
  if [ $stage -le 12 ]; then
    ...
    steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
        ...
        --cmd "$train_cmd" 7000 $data_root/${train_set}_sp $lang $lat_dir $treedir || exit 1
  fi
else
