...
if [ $stage -le 2 ]; then
  utils/data/perturb_data_dir_speed_3way.sh $data_root/${unsupervised_set} \
    $data_root/${unsupervised_set_perturbed}
  steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj \
    $data_root/${unsupervised_set_perturbed}
  steps/compute_cmvn_stats.sh $data_root/${unsupervised_set_perturbed}
  utils/fix_data_dir.sh $data_root/${unsupervised_set_perturbed}
  # we have to combine short segments or we won't be able to train chain models
  # on those segments.
  utils/data/combine_short_segments.sh \
  $data_root/${unsupervised_set_perturbed} 1.55 $data_root/${unsupervised_set_perturbed}_comb
...
if [ $stage -le 9 ]; then
  ...
  steps/nnet3/chain/build_tree_multiple_sources.sh --stage -5 \
...
if [ $stage -le 11 ]; then
  ...
  output-layer name=output-0 input=output.affine
  output-layer name=output-1 input=output.affine
  output-layer name=output-0-xent input=output-xent.log-softmax
  output-layer name=output-1-xent input=output-xent.log-softmax
EOF
