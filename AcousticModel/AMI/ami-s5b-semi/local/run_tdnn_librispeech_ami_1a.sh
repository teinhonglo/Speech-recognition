...
train_stage=-10
get_egs_stage=-10
dir=$exp_root/chain/tdnn_librispeech_ami_1a
xent_regularize=0.1

# configs for transfer learning
src_mdl=../../Librispeech/s5/exp/chain/tdnn1d_sp/final.mdl # Input chain model
...
nnet_affix=_online_Librispeech
...
ali_dir=$exp_root/tri3b_ali
treedir=$exp_root/chain/tri4_5n_tree
...
if [ $stage -le 4 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" $data_root/train \
    data/lang $exp_root/tri3b $exp_root/tri3b_lats || exit 1;
  rm $exp_root/tri3b_lats/fsts.*.gz 2>/dev/null || true # save space
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
    ...
    --cmd "$train_cmd" 1200 $data_root/train $lang $ali_dir $treedir || exit 1;
fi
...
if [ $stage -le 8 ]; then
  echo "$0: generate egs for chain to train new model on rm $data_rootset."
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
    utils/create_split_dir.pl \
     /$exp_rootort/b0{3,4,5,6}/$USER/kaldi-$data_root/egs/rm-$(date +'%m_%d_%H_%M')/s5/$dir/egs/storage $dir/egs/storage
  fi
  ivector_dir=
  if $use_ivector; then ivector_dir="$exp_root/nnet2${nnet_affix}/ivectors" ; fi
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    ...
    --feat-dir $data_root/train_hires \
    --tree-dir $treedir \
    --lat-dir $exp_root/tri3b_lats \
    --dir $dir || exit 1;
fi
...
if [ $stage -le 9 ]; then
  ...
  ivec_opt=""
  if $use_ivector;then
    ivec_opt="--online-ivector-dir $exp_root/nnet2${nnet_affix}/ivectors_test"
  fi
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang $dir $dir/graph
  steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
    --scoring-opts "--min-lmwt 1" \
    --nj 20 --cmd "$decode_cmd" $ivec_opt \
    $dir/graph $data_root/test_hires $dir/decode || exit 1;
fi
wait;
exit 0;
