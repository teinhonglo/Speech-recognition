...
if [ $stage -le 4 ]; then
  ...
  if [ ! -f ${norvb_datadir}${rvb_affix}${num_data_reps}_hires_nopitch/feats.scp ]; then
    ...
    steps/make_mfcc_pitch.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
                             --cmd "$train_cmd --max-jobs-run $nj" ${norvb_datadir}${rvb_affix}${num_data_reps}
    steps/compute_cmvn_stats.sh ${norvb_datadir}${rvb_affix}${num_data_reps}
    utils/fix_data_dir.sh ${norvb_datadir}${rvb_affix}${num_data_reps}
    # high resolution
    utils/data/perturb_data_dir_volume.sh ${norvb_datadir}${rvb_affix}${num_data_reps}_hires
    steps/make_mfcc_pitch.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
                     --cmd "$train_cmd --max-jobs-run $nj" ${norvb_datadir}${rvb_affix}${num_data_reps}_hires
    ...
    # create MFCC data dir without pitch to extract iVector
    utils/data/limit_feature_dim.sh 0:39 ${norvb_datadir}${rvb_affix}${num_data_reps}_hires \
    	                                 ${norvb_datadir}${rvb_affix}${num_data_reps}_hires_nopitch || exit 1;
    steps/compute_cmvn_stats.sh ${norvb_datadir}${rvb_affix}${num_data_reps}_hires_nopitch || exit 1;
    utils/fix_data_dir.sh ${norvb_datadir}${rvb_affix}${num_data_reps}_hires_nopitch
  fi
  utils/combine_data.sh data/${train_set}_sp${rvb_affix} data/${train_set}_sp ${norvb_datadir}${rvb_affix}${num_data_reps}
  utils/combine_data.sh data/${train_set}_sp${rvb_affix}_hires data/${train_set}_sp_hires \
  	                ${norvb_datadir}${rvb_affix}${num_data_reps}_hires
  utils/combine_data.sh data/${train_set}_sp${rvb_affix}_hires_nopitch data/${train_set}_sp_hires_nopitch \
  	                ${norvb_datadir}${rvb_affix}${num_data_reps}_hires_nopitch
fi
...
if [ $stage -le 5 ]; then
  ...
  num_utts_total=$(wc -l <data/${train_set}_sp${rvb_affix}_hires_nopitch/utt2spk)
  num_utts=$[$num_utts_total/4]

  utils/data/subset_data_dir.sh data/${train_set}_sp${rvb_affix}_hires_nopitch \
     $num_utts ${temp_data_root}/${train_set}_sp${rvb_affix}_hires_nopitch_subset

  echo "$0: computing a PCA transform from the hires data."
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      --splice-opts "--left-context=3 --right-context=3" \
      --max-utts 10000 --subsample 2 \
       ${temp_data_root}/${train_set}_sp${rvb_affix}_hires_nopitch_subset \
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    --num-frames 700000 \
    --num-threads 8 \
    ${temp_data_root}/${train_set}_sp${rvb_affix}_hires_nopitch_subset 512 \
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 --num-processes 1 --num-threads 5 \
     data/${train_set}_sp${rvb_affix}_hires_nopitch exp/nnet3${nnet3_affix}/diag_ubm \
...
if [ $stage -le 7 ]; then
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${train_set}_sp${rvb_affix}_hires_nopitch ${temp_data_root}/${train_set}_sp${rvb_affix}_hires_nopitch_max2
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd --num-threads 3" --nj 30 \
    ${temp_data_root}/${train_set}_sp${rvb_affix}_hires_nopitch_max2 \
